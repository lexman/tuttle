...
Before we draw the graph, we'll have to respond to our curious cooker who's very interested in our data. We have sent
him the `characters_count.dat` file, but he can't open it with his favorite spreadsheet software.

According to the doc, strings should be quoted, columns needs to be separated by a comma instead of a tabulation... And
...
to commit the `tuttlefile`, and `Les_trois_mousquetaires.zip` because it is a *primary resource*. Other *resources* like
`Les_trois_mousquetaires.txt` or `characters_count.dat` can always be re-generated by tuttle, from te `tuttlefile` and
the primary resources. You can even re-generate them as they used to be, from your source control history.

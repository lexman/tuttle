...
Now is the time to commit your work in your versioning system, eg `git`, if you haven't done it already.
...
[the digitalized text is still being reviewed by volunteers](https://fr.wikisource.org/wiki/Livre:Dumas_-_Les_Trois_Mousquetaires_-_1849.djv)
...
The drawback of using remote resources, is that we don't always control them. What will happen it the guy that published the zip
updates it while we are working on the gnuplot part ? The whole the workflow could be invalidated !

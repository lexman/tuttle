#!/usr/bin/env python
#
# This file copied from the project https://github.com/tornadoweb/tornado is used as a mock for integration tests to s3 server
#
...
def start(port, root_directory="/tmp/s3", bucket_depth=0):
    ...
    theloop = ioloop.IOLoop.current()
    theloop.start()
    return theloop


def stop(theloop):
    theloop.add_callback(theloop.stop)
